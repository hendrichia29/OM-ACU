...
Public Class FormTrPOManagementKlemJadi
    ...
    Private Sub setData()
        Try
            ...
            Dim reader = execute_reader(" Select MB.KdBarang, NamaBarang, " & _
                                        ...
                                        " Join MsBarang MB On PO.KdBahanMentah = MB.KdBarang " & _
                                        " where No_PO = '" & PK & "' " & _
    ...
    Public Sub setCmbBarang()
        ...
        Dim reader = execute_reader(" Select KdBarang, NamaBarang " & _
    ...
    Private Sub FormTrPOManagement_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If type_po = "klem_jadi" Then
            Me.Text = "Pembelian Klem Jadi"
        Else
            Me.Close()
        End If
        clear_variable_array()
        gridBarang.Columns("clmQtyReal").HeaderText = "Jumlah ( Pack )"
    ...
    Function SavePODetail(ByVal flag As String)
        ...
        For i As Integer = 0 To gridBarang.RowCount - 1
            ...
            If flag = 1 Then
                StockBarang(Qty_real, OP, harga, KdBarang, Attribute, Trim(txtID.Text), "Form Penerimaan Klem Jadi")
            End If
            ...
            Try
                ...
                If status = "add" Then
                    ...
                    sql = " insert into trheaderpb ( " & _
                          ...
                          " GrandTotal, jenis_pb, Disc, Jumlah " & _
                          " ) values ( " & _
                          " '" + Trim(txtID.Text) + "', '" & Trim(txtID.Text) & "', " & _
                          " '" & kdKaryawan & "', '" & Trim(supplierID(0)) & "', " & _
                          " '" + dtpPO.Value.ToString("yyyy/MM/dd HH:mm:ss") + "', " & _
                          " '" & Trim(flag) & "', '" & Trim(Grandtotal) & "', " & _
                          " '" & type_po & "', '" & txt_disc_faktur.Text & "', " & _
                          " '" & Jumlah & "' ) "
                    execute_update_manual(sql)
                ...
                deleteTabelTemp()
                trans.Commit()
                msgInfo("Data berhasil disimpan. Silakan cetak bukti penerimaan barang")
                Me.Close()
            Catch ex As Exception
                trans.Rollback()
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        Try
            If cmbBarang.SelectedIndex = 0 Then
                ...
                cmbBarang.Focus()
            ElseIf Val(txtQtyPack.Text) = 0 Then
            ...
            Else
                ...
                For i As Integer = 0 To (gridBarang.RowCount - 1)
                    If gridBarang.Rows(i).Cells("clmPartNo").Value.ToString = Trim(barangID(0)) Then
                        Dim addQty = Val(txtQtyPack.Text)
                        ...
                        gridBarang.Rows.Item(i).Cells("clmQtyReal").Value = addQty
                        gridBarang.Rows.Item(i).Cells("clmHargaBeli").Value = FormatNumber(CDbl(txtHargaBeli.Text), 0)
                        gridBarang.Rows.Item(i).Cells("clmSubtotal").Value = FormatNumber(CDbl(txtHargaBeli.Text) * (addQty), 0)
                        emptyBarang()
                        gridBarang.Focus()
                        HitungTotal()
                        Exit Sub
                    End If
                Next
                Dim Subtotal = (Val(harga) * Val(txtQtyPack.Text))
                gridBarang.Rows.Add()
                ...
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells("clmQtyReal").Value = Val(txtQtyPack.Text)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells("clmHargaBeli").Value = FormatNumber(CDbl(txtHargaBeli.Text), 0)
                gridBarang.Rows.Item(gridBarang.RowCount - 1).Cells("clmSubtotal").Value = FormatNumber(CDbl(txtHargaBeli.Text) * (Val(txtQtyPack.Text)), 0)
                emptyBarang()
                gridBarang.Focus()
                HitungTotal()
            End If
    ...
    Private Sub cmbBarang_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbBarang.SelectedIndexChanged
        If cmbBarang.SelectedIndex <> 0 Then
            ...
            sql = " SELECT IFNULL(SUM(MBL.Qty),0) StockAkhir, " & _
                  " IFNULL(MBL.Harga,0) AS HargaAwal, " & _
                  " NamaBarang, Ukuran, satuan " & _
                  " FROM MsBarang MB " & _
                  " LEFT JOIN MsBarangList MBL ON MBL.KdBarang = MB.KdBarang " & _
                  " WHERE MB.isAktif = 1 " & _
    ...
    Private Sub gridBarang_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridBarang.CellEndEdit
        Try
            Dim harga = 0
            Dim qty = Val(gridBarang.CurrentRow.Cells("clmQtyReal").Value)
            Dim HargaBeli = gridBarang.CurrentRow.Cells("clmHargaBeli").Value
            If Val(qty) <= 0 Then
                MsgBox("Jumlah harus lebih besar dari 0", MsgBoxStyle.Information, "Validation Error")
                qty = 1
                gridBarang.CurrentRow.Cells("clmQtyReal").Selected = True
            ...
            gridBarang.CurrentRow.Cells("clmQtyReal").Value = Val(qty)
            gridBarang.CurrentRow.Cells("clmSubtotal").Value = FormatNumber(CDbl(HargaBeli) * (qty), 0)
            HitungTotal()
        Catch ex As Exception
            MsgBox(ex.ToString, MsgBoxStyle.Critical, "Warning!!!")
        End Try
    ...
    Private Sub txtQtyPack_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtQtyPack.KeyPress
        If Asc(e.KeyChar) <> Asc("-") And AscW(e.KeyChar) <> 13 And AscW(e.KeyChar) <> 8 And (Asc(e.KeyChar) < Asc("0") Or Asc(e.KeyChar) > Asc("9")) Then
            e.KeyChar = Nothing
        End If
        If AscW(e.KeyChar) = 13 And txtQtyPack.Text <> "" Then
            txtHargaBeli.Focus()
        End If
    End Sub
End Class