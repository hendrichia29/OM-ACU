...
Public Class FormLapPO
    ...
    Private Sub initGrid(ByVal s As String, ByVal s2 As String) 'area,toko
        ...
        query = " select ho.No_PO `No Pemesanan`,DATE_FORMAT(Tanggal_PO,'%d-%m-%Y') `Tgl Pemesanan`,c.Nama `Supplier`,mp.KDBahanMentah  `Kode Bahan Mentah`,NamaBahanMentah   " & _
        ", jumlah `Qty`  " & _
        ", CASE WHEN StatusPO = 0 THEN 'New' WHEN StatusPO = 1 THEN 'Confirm' " & _
        " WHEN StatusPO = 2 THEN 'Barang Diterima' End 'Status PO' "
        query2 = " select ho.No_PO `No Pemesanan`,DATE_FORMAT(Tanggal_PO,'%d-%m-%Y') `Tgl Pemesanan`,c.Nama `Supplier`,mp.KDBahanMentah  `Part No.`,NamaBahanMentah  NamaBarang " & _
               ", jumlah `Qty`  " & _
               ", CASE WHEN StatusPO = 0 THEN 'New' WHEN StatusPO = 1 THEN 'Confirm' " & _
               " WHEN StatusPO = 2 THEN 'Barang Diterima' End 'Status PO' "
        queryFrom = " from trheaderpo ho join trdetailpo do  on ho.no_po=do.no_po join MsbahanMentah mp on mp.KDBahanMentah=do.KDBahanmentah " & _
        ...
        Try
            tglMulai = tg1
            tglAkhir = tg2
            dropview("viewCetakLapPO" & kdKaryawan)
            createview(query2, "viewCetakLapPO" & kdKaryawan)
        ...
        End Try
    End Sub
    Private Sub FormLapPO_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
